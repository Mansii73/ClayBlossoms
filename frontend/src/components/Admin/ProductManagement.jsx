...
const ProductManagement = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Failed to fetch products. Please check if the backend server is running.';
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      const errorMessage = err.response?.data?.message || (currentProduct ? 'Failed to update product' : 'Failed to add product');
      setError(errorMessage);
      toast.error(errorMessage);
    }
  };
